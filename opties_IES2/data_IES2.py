...
def import_data(path="data/"):
    ...
    buses_IES2 = pd.read_csv(path+'Erw/buses_IES2.csv').set_index('name')
    buses_IES2['geometry'] = buses_IES2['geometry'].apply(shapely.wkt.loads)
    buses_IES2 = gpd.GeoDataFrame(buses_IES2, geometry = 'geometry')

    lines = pd.read_csv(path + "lines_IES2.csv", delimiter=";").set_index("id")
    generators = pd.read_csv(path + "generatorsIES2_nachEEG.csv", delimiter=";").set_index("name")
    storage_units = pd.read_csv(path + "storage_units_IES2.csv", delimiter=";").set_index("name")
    stores = pd.read_csv(path + "stores_IES2.csv", delimiter=";").set_index("name")
    links = pd.read_csv(path + "links_IES2_nachEEG.csv", delimiter=";").set_index("name")
    loads = pd.read_csv(path + "loads_IES2.csv").set_index("name")

    loads = loads.drop(["LS1", "LS2"])

    return buses, buses_IES2, lines, generators, storage_units, stores, links, loads


def import_timeseries(path="data/timeseries/"):
    el_loads = pd.read_csv(path + "el_profiles_IES2+WP+E-Mob.csv", delimiter=";").set_index("time")
    el_loads.index = pd.date_range("2019-01-01 00:00", "2019-12-31 23:00", freq="H")

    heat_load = pd.read_csv(path + "heat_load_slp.csv", delimiter=";").set_index("time")
    heat_load.index = pd.date_range("2019-01-01 00:00", "2019-12-31 23:00", freq="H")

    gas_load = pd.read_csv(path + "gas_load.csv", delimiter=",").set_index("time")
    ...
    wind = pd.read_csv(path + "wind_timeseries_ab250.csv", delimiter=";")
    wind = pd.Series(
...
def create_pypsa_network(
    ...
    for i in range(0, len(buses_IES2)):
        bus = buses_IES2.iloc[i]
        network.add(
            "Bus", 
            name=bus.name, 
            carrier=bus.carrier, 
