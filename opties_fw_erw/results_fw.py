...
def calc_investment_cost(network):
    ...
    if not ext_store.empty:
        ...
        sto_costs[2] = ((gas1.e_nom_opt - gas1.e_nom_min) * gas1.capital_cost).sum()
        gas2 = ext_store[ext_store.index == "GSp2"]
        sto_costs[3] = ((gas2.e_nom_opt - gas2.e_nom_min) * gas2.capital_cost).sum()
...
#
    ...
    results.value["annualisierte Investkosten Biogasspeicher 1"] = invest[2][2]
    
    results.value["annualisierte Investkosten Biogasspeicher 2"] = invest[2][3]
  ...
  #        .groupby(network.generators.carrier)  
    ...
    results.value["Wärmelast (Wärmenetz)"] = -network.links_t.p1["HT_1"].sum().sum()-network.links_t.p1["HT_72"].sum().sum()
