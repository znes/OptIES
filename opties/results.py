...
def calc_results(network):
    ...
    results = pd.DataFrame(
        ...
        index=[
            ...
            "Kosten aus Betrieb der BHKWs (inklusive Biogaserzeugung)",
            "Betrieb: ",
            "Biogaserzeugung",
    ...
    results.unit[results.index.str.contains("Netzbezug")] = "MW"
    results.unit[results.index.str.contains("Netzeinspeisung")] = "MW"
    ...
    results.unit[results.index.str.contains("ausbau")] = "MW"
    results.unit[results.index.str.contains("Last")] = "MW"
    results.unit[results.index.str.contains("last")] = "MW"
    results.unit[results.index.str.contains("restlich")] = "MW"
    results.unit[results.index.str.contains("Erzeugung")] = "MW"
    results.unit[results.index.str.contains("erzeugung")] = "MW"
    ...
    results.value[
        "annualisierte Investkosten diverser Link-Komponenten"
    ] = invest[1]
    ...
    results.value[
        "annualisierte marginale Kosten"
    ] = calc_marginal_cost(network)
    
    results.value[
        "Erträge aus Trocknungsanlage"
    ] = network.links_t.p0['TA'].mul(
        network.snapshot_weightings.objective, axis=0).sum() \
            * (network.links.loc['TA'].marginal_cost)
    
    results.value[
        "Erträge aus Netzeinspeisung"
    ] = network.links_t.p0['NA_Sp'].mul(
        network.snapshot_weightings.objective, axis=0).sum() \
            * (network.links.loc['NA_Sp'].marginal_cost)
    
    results.value[
        "Kosten aus Netzbezug"
    ] = network.generators_t.p['NeAn'].mul(
        network.snapshot_weightings.objective, axis=0).sum() \
            * (network.generators.loc['NeAn'].marginal_cost)
            
    results.value[
        "Kosten aus Betrieb der BHKWs (inklusive Biogaserzeugung)"
    ] = network.generators_t.p['BGA'].mul(
        network.snapshot_weightings.objective, axis=0).sum() \
            * (network.generators.loc['BGA'].marginal_cost) \
        + (network.links_t.p0[network.links[network.links.carrier=='KWK_AC'].index].mul(
            network.snapshot_weightings.objective, axis=0).sum() \
                * (network.links[network.links.carrier=='KWK_AC'].marginal_cost)).sum()
    ...
    results.value[
        "rel. Netzausbau"
    ] = calc_network_expansion(network)[0].sum() / ext_lines.s_nom.sum()
    
    results.value[
        "Ausbau Batteriespeicher"
    ] = (network.storage_units.p_nom_opt - network.storage_units.p_nom_min) \
        [network.storage_units.p_nom_extendable].groupby(network.storage_units.carrier).sum().sum()
    ...
    results.value[
        "gesamte elektrische Last"
    ] = network.loads_t.p[network.loads[network.loads.carrier=='AC'].index].sum().sum()
    
    results.value[
        "gesamte Wärmelast"
    ] = network.loads_t.p[network.loads[network.loads.carrier=='heat'].index].sum().sum()
    
    results.value[
        "Erzeugung aus PV-Anlagen"
    ] = network.generators_t.p[network.generators[network.generators.carrier=='PV'].index].sum().sum()
    
    results.value[
        "Erzeugung durch BHKW - Strom"
    ] = abs(network.links_t.p1[network.links[network.links.carrier=='KWK_AC'].index].sum().sum())
    
    results.value[
        "Erzeugung durch BHKW - Wärme"
    ] = abs(network.links_t.p1[network.links[network.links.carrier=='KWK_heat'].index].sum().sum())
    
    results.value[
        "Erzeugung durch Spitzenlastkessel"
    ] = network.generators_t.p['SpK'].sum()
    
    results.value[
        "Last der Trocknungsanlage"
    ] = network.links_t.p0['TA'].sum()
    
    results.value[
        "restliche Abwärme"
    ] = network.links_t.p0['Abw'].sum()
    
    results.value[
        "Netzbezug"
    ] = network.generators_t.p['NeAn'].sum()
    
    results.value[
        "Netzeinspeisung"
    ] = network.links_t.p0['NA_Sp'].sum()
    
    results.value[
        "Biogaserzeugung"
    ] = network.generators_t.p['BGA'].sum()
