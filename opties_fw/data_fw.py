...
def import_data(path='data/'):
    ...
    buses_fw = pd.read_csv(path+'Erw/buses_fw.csv').set_index('name')
    buses_fw['geometry'] = buses_fw['geometry'].apply(shapely.wkt.loads)
    buses_fw = gpd.GeoDataFrame(buses_fw, geometry = 'geometry')
    ...
    links_fw = pd.read_csv(path+'Erw/links_fw.csv', delimiter=";").set_index('name')

    loads = pd.read_csv(path+'loads_fw.csv').set_index('name')
    loads = loads.drop(['LS1', 'LS2']) # TODO: noch ohne E-Mob

   #th_loads = pd.read_csv(path+'Erw/th_loads.csv').set_index('name')

    return buses, buses_fw, lines, generators, storage_units, stores, links, links_fw, loads
...
def import_timeseries(path='data/timeseries/'):
    ...
    return el_profiles, th_profiles, gas_profile, pv
...
def create_pypsa_network(
    ...
    for i in range(0, len(links_fw)):
        link = links_fw.iloc[i]
        network.add(
            "Link", 
            name = link.name,
            bus0=link.bus0, 
